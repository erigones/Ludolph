...
class ZabbixAPI(object):
    ...
    def get_severity(self, prio):
        """
        Return severity string from severity id.
        """
        try:
            return self.TRIGGER_SEVERITY[int(prio)]
        except KeyError:
            return 'unknown'
    ...
    def get_age(self, dt):
        """
        Calculate delta between current time and datetime and return a
        human readable form of the delta object.
        """
    ...
    def debug(self, level, var="", msg=None, safe=False):
        if msg:
            strval = str(msg)
        if var != "":
            strval = str(var)
        ...
        if safe:
            # Do not log passwords and tokens
            strval = re.sub(r'("password": )".*?"', r'\1"***"', strval)
            strval = re.sub(r'("auth": )".*?"', r'\1"***"', strval)
    ...
    def do_request(self, json_obj):
        ...
        self.debug(logging.INFO, "Sending: " + str(json_obj), safe=True)
